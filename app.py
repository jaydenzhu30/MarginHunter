...
def team_roster(team_id):
    try:
        # Try season averages first to get only active players
        for season in [2024, 2023]:
            try:
                ...
                players = avg_data.get("data", [])
                if players:
                    # Build player objects from averages data
                    result = []
                    for p in players:
                        player = p.get("player", {})
                        if player:
        ...
        # Fallback: get players by team
        data = bdl_get("/players", {"team_ids[]": team_id, "per_page": 100})
        return jsonify(data.get("data", []))
    except Exception as e:
        return jsonify({"error": str(e)}), 500
